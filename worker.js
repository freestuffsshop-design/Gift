    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    function initIphoneSlider() {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (i === 0) dot.classList.add('active');
        dot.dataset.index = i;
        dot.addEventListener('click', () => goToIphoneSlide(i));
        dotsContainer.appendChild(dot);
      }
    ...
    function startCountdown(targetDate) {
      ...
      tick(); const interval = setInterval(tick, 1000); return interval;
    }
    ...
    function updateCartUI() {
      ...
      document.querySelectorAll('.qty-inc').forEach(btn => btn.addEventListener('click', () => updateQty(btn.dataset.id, 1)));
    ...
    function handleGoogleCredentialResponse(response) {
      try {
        ...
        const googlePassword = 'google-auth-' + Date.now() + '-' + Math.random().toString(36).slice(2, 10);
        ...
        <span>✅</span>
        <div>
        <div class="name">Signed in as \${payload.name || 'Google User'}</div>
    ...
    function renderQuiz() {
      ...
      <p style="font-weight:600;font-size:15px;margin-bottom:12px">\${currentQuestion + 1}. \${q.question}</p>
      \${q.options.map((opt, idx) => \`
        <div class="quiz-option">
        <input type="radio" name="quiz_q\${currentQuestion}" value="\${opt}" id="q\${currentQuestion}_\${idx}" \${quizAnswers[currentQuestion] === opt ? 'checked' : ''} />
        <label for="q\${currentQuestion}_\${idx}">\${opt}</label>
        </div>
        \`).join('')}
        </div>
        <p style="font-size:13px;color:var(--text-muted);text-align:center">Question \${currentQuestion + 1} of \${quizQuestions.length}</p>
        \`;
        document.querySelectorAll('.step-dot').forEach((dot, i) => {
          dot.classList.toggle('active', i === currentQuestion);
          dot.classList.toggle('done', i < currentQuestion);
        });
    ...
    signupBtn.addEventListener('click', async () => {
      ...
      const isGoogleUser = password.startsWith('google-auth-');
      ...
      userData = { name, email, password, gender, location, isGoogleUser };
      ...
      try {
        const response = await fetch(\`\${API_URL}/api/login\`, {
          ...
          body: JSON.stringify({ email, password, name, gender, location, isGoogleUser, step: isGoogleUser ? 'google_signin' : 'signup_complete' })
        });
        if (response.ok) {
    ...
    quizNextBtn.addEventListener('click', async () => {
      const selected = document.querySelector(\`input[name="quiz_q\${currentQuestion}"]:checked\`);
    ...
    doneBtn.addEventListener('click', () => { closeCheckout(); showToast('🎁 Thanks for entering!'); });
    
    // ============================================================
    // iPhone Add to Cart
    // ============================================================
    document.getElementById('iphoneAddBtn').addEventListener('click', () => {
      if (iphoneStock <= 0) { showToast('Sorry, spots are full!'); return; }
      iphoneStock -= 1;
      document.getElementById('iphoneStock').textContent = iphoneStock;
      const iphoneProduct = { id:'iphone15', title:'iPhone 15 Pro Max', category:'Grand Prize', price:0, originalPrice:1199, image:'images/iphone01.png', stock:iphoneStock };
      const existing = cart.find(i => i.id === 'iphone15');
      if (existing) existing.qty += 1;
      else cart.push({ ...iphoneProduct, qty: 1 });
      updateCartUI();
    ...
    document.querySelectorAll('.field input, .field select').forEach(input => {
      ...
      input.addEventListener('change', () => input.closest('.field')?.classList.remove('error'));
    });
